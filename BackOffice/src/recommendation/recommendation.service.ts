...
export class RecommendationService {
  ...
  private async _doGenerate(
    ...
    try {
      ...
      if (err.response) {
        try {
          this.logger.warn(`[Ollama] HTTP ${err.response.status} — ${JSON.stringify(err.response.data).slice(0, 300)}`);
        } catch { this.logger.warn(`[Ollama] HTTP ${err.response.status} — (response non sérialisable)`); }
      }
      parsed = {
    ...
    // Scénario 1 — Notifier le manager créateur de l'activité
    try {
      const createdById = (activity as any).createdById;
      if (createdById) {
        await this.notifService.notifyRecommendationReady(
          String(createdById),
      ...
      this.logger.warn(`[Notif] Scénario 1 failed: ${err.message}`);
    }
  ...
  async saveDecision(
    ...
    // Si approuvé → ajouter les required skills + notifier l'employé
    if (body.decision === 'approved') {
      try {
        ...
        this.logger.warn(`[Decision] Failed to update employee skills/notif: ${err.message}`);
      }
    }
